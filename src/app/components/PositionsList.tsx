...
export function PositionsList({ title, positions, onSelectPosition }: PositionsListProps) {
  return (
    <div className="mb-8">
        ...
        <table className="w-full">
          ...
          <tbody>
            {positions.map((position) => {
              const purchasePrice = Number.isFinite(position.purchasePrice) ? position.purchasePrice : 0;
              const currentPrice = Number.isFinite(position.currentPrice) ? position.currentPrice : 0;
              const totalValue = Number.isFinite(position.totalValue) ? position.totalValue : 0;
              const change = Number.isFinite(position.change) ? position.change : 0;
              const changePercent = Number.isFinite(position.changePercent) ? position.changePercent : 0;
              ...
              return (
                ...
                <td className="py-4 px-2 text-right">
                  <div className="text-sm">
                    {purchasePrice.toLocaleString('ru-RU')} ₽ → {currentPrice.toLocaleString('ru-RU')} ₽
                  </div>
                </td>
                <td className="py-4 px-2 text-right">
                  <div className="text-sm mb-0.5">{totalValue.toLocaleString('ru-RU')} ₽</div>
                  <div className="text-xs text-gray-500">{position.amount} шт.</div>
                </td>
                <td className="py-4 px-2 text-right">
                  <div className={`text-sm mb-0.5 ${change >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                    {change >= 0 ? '+' : ''}{change.toLocaleString('ru-RU')} ₽
                  </div>
